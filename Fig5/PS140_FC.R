...
df <- df %>%
  mutate(
    treatment = factor(as.character(treatment),
                       levels = treat_levels[treat_levels %in% unique(as.character(treatment))])
  )
...
# Deriving regime and inoculant from treatment (for ANOVA)
df <- df %>%
  mutate(
    regime    = factor(str_extract(as.character(treatment), "^[A-Z]{2}"),
                       levels = c("NW","WW")[c("NW","WW") %in% str_extract(levels(treatment), "^[A-Z]{2}")]),
    inoculant = factor(str_replace(as.character(treatment), "^[A-Z]{2}_", ""),
                       levels = c("MIX1","MIXR","NOI")[c("MIX1","MIXR","NOI") %in% str_replace(levels(treatment), "^[A-Z]{2}_", "")])
  )

# Index
vars_plot <- c("FvFm","FvFo","PhiPSII")

# Labelling Y
ylab_map <- list(
  FvFm    = expression(F[V] / F[M]),
  FvFo    = expression(F[V] / F[0]),
  PhiPSII = expression(Phi[PSII])
)
...
#   Combined Figure
# ==========================================================
if (all(c("FvFm","FvFo","PhiPSII") %in% names(plots_list))) {
  
  p_comb <- ggpubr::ggarrange(
    plots_list[["FvFm"]]    + theme(legend.position = "none"),
    plots_list[["FvFo"]]    + theme(legend.position = "none"),
    plots_list[["PhiPSII"]] + theme(legend.position = "none"),
    ncol = 1,
    nrow = 3,
    common.legend = TRUE,
    legend = "bottom",
  ...
  ggsave(
    file.path("plots_selected", "scatter_FC_combined_vertical_FvFm_FvFo_PhiPSII.png"),
    plot = p_comb,
    width = 7,
    height = 14,
    dpi = 300
  ...
  ggsave(
    file.path("plots_selected", "scatter_FC_combined_vertical_FvFm_FvFo_PhiPSII.pdf"),
    plot = p_comb,
    width = 7,
    height = 14,
    device = cairo_pdf
  ...
  message("Combined figure saved in plots_selected/")
}

# ==========================================================
# Two-Way ANOVA (regime * inoculant) day-by-day
# for: FvFm, FvFo, PhiPSII
...
for (met in metrics_available) {
  ...
  cat("============================================================\n")
  for (d in days) {
    dd <- df %>% filter(n_day == d)
    print_aov_2way(dd, metric = met, day_label = d, nice_name = nice_names[[met]] %||% met)
  }
}
cat("\nDone: Two-way ANOVA tables printed in console for each index and day.\n")
...
# Index
metrics_1way <- intersect(c("FvFm","FvFo","PhiPSII"), names(df))
  ...
  if (is.finite(p_val) && p_val < 0.05) {
    ...
    if (!is.null(hs) && !is.null(hs$groups)) {
      ...
      out_letters <- means_tbl %>%
        ...
        arrange(desc(mean))
      cat("\nTukey HSD (inoculant) — significant letters:\n")
...
days_1way   <- sort(unique(df$n_day))
regimes_avl <- intersect(c("NW","WW"), levels(df$regime))

for (met in metrics_1way) {
  cat("\n============================================================\n")
  cat(sprintf("Index: %s\n", nice_names_1way[[met]] %||% met))
  cat("============================================================\n")
  for (reg in regimes_avl) {
    for (d in days_1way) {
      ...
      run_oneway_in_regime(dd, metric = met, day_label = d,
                           regime_label = reg,
                           pretty_metric = nice_names_1way[[met]] %||% met)
    }
  }
}
cat("\nDone: One-Way ANOVA per regime (WN/WW) printed in console for each index and day.\n")