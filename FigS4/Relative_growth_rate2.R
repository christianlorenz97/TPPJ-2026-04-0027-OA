...
# File load + import
file_index      <- "PS141_index.txt"
file_index_2way <- "PS141_index_2way.txt"
...
global_max <- df2 %>%
  summarise(
    root_tot_length_max  = max(root_tot_length,  na.rm = TRUE),
    root_tot_area_max    = max(root_tot_area,    na.rm = TRUE),
    shoot_height_max     = max(shoot_height,     na.rm = TRUE),
    shoot_side_area_max  = max(shoot_side_area,  na.rm = TRUE)
  )

G_root_len  <- global_max$root_tot_length_max
G_root_area <- global_max$root_tot_area_max
...
write_csv(global_max, file.path("tables_index_RESI_meanse", "PS141_RESI_global_max.csv"))
...
compute_resi_one <- function(df_one, time_col = "n_day", value_col, global_max) {
  ...
  # useful for lowers >1 due to numeric noise
...
indices_long <- df2 %>%
  ...
  purrr::map_dfr(function(df_one) {
    tibble(
      ...
      Root_Elongation_Rate =
        compute_resi_one(df_one, value_col = "root_tot_length", global_max = G_root_len),
      
      Root_Area_Enlargement_Rate =
        compute_resi_one(df_one, value_col = "root_tot_area", global_max = G_root_area),
...
write_csv(
  indices_long,
  file.path("tables_index_RESI_meanse", "PS141_RESI_all_indices_long.csv")
)
...
make_barplot_index <- function(df_idx, var, ylab_txt) {
  ...
  sum_tbl <- sum_tbl %>%
    mutate(
      y_lab = pmin(mean + se + 0.03, 0.95)
    )
  
  y_for_symbol <- 0.97  # symbol right above
...
# Barplot 
ylabs_resi <- c(
  Root_Elongation_Rate             = "Root Elongation Rate",
  Root_Area_Enlargement_Rate       = "Root Area Enlargement Rate",
...
combined_resi <- (res_plots[["Root_Elongation_Rate"]]$plot +
                    res_plots[["Root_Area_Enlargement_Rate"]]$plot) /
  (res_plots[["Shoot_Elongation_Rate"]]$plot +
     res_plots[["Shoot_Side_Area_Enlargement_Rate"]]$plot) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")

dir.create("plots_index_RESI", showWarnings = FALSE)
ggsave("plots_index_RESI/PS141_RESI_barplots_grid.tiff",
       plot   = combined_resi,
       width  = 12,
       height = 10,
       dpi    = 300,
...
write_meanse_tukey_RESI <- function(df_idx, var, letters_tbl) {
  ...
  sum_tbl <- df_idx %>%
    ...
    mutate(
      cell = dplyr::case_when(
        is.finite(se) & !is.na(letters) & letters != "" ~ paste0(mean, " ± ", se, " ", letters),
  ...
  write_csv(out, file.path("tables_index_RESI_meanse",
                           paste0("mean_se_Tukey_RESI_", var, ".csv")))
}
