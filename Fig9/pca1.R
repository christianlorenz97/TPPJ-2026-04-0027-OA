...
cor(data_mv[,2:16]) #default pearson

col <- colorRampPalette(c("#BB4444", "#EE9988", "#FFFFFF", "#77AADD", "#4477AA"))
corrplot(cor(data_mv[,2:16], method="pearson"), method="circle", col=col(200),  
         type="upper",
...
# Correlation scatterplot with corrplot
data_mv<-read.delim("mv_matrix_2.txt")

cor(data_mv[,2:8]) #default pearson

col <- colorRampPalette(c("#BB4444", "#EE9988", "#FFFFFF", "#77AADD", "#4477AA"))
corrplot(cor(data_mv[,2:8], method="pearson"), method="circle", col=col(200),  
         type="upper",
         addCoef.col = "black", # Add coefficient of correlation
         tl.col="black", tl.srt=45, #Text label coslor and rotation
)
...
# Import
data_mv1 <- read.delim("mv_matrix_2.txt")
...
# PCA
pca <- prcomp(data_mv1[2:8], center = TRUE, scale. = TRUE)
...
fit_man <- manova(
  cbind(FvFm, PhiPSII, fSH, rDW, sDW, hRAE, hRSA) ~ Treatment,
  data = data_mv1
)
...
### MANOVA Two-Way ###
data_mv1 <- read.delim("mv_matrix2_2way.txt")
var_num<-cbind(data_mv1$FvFm,
               data_mv1$PhiPSII,
               data_mv1$fSH,
               data_mv1$rDW,
...
# MANOVA — multivariate vs Treatment
fit_man <- manova(
  cbind(FvFm, PhiPSII, fSH, rDW, sDW, hRAE, hRSA) ~ Regime * Inoculant,
  data = data_mv1
)
